    ...
    if (res == SZ_OK)
    {
        ...
        */
        ...
        for (unsigned i = 0; i < db.NumFiles; i++)
        {
            ...
            if (_wcsicmp(fullname + namelen, L".inf") == 0 || _wcsicmp(fullname + namelen, L".cat") == 0)
            {
                // Log.print_con("{");
            ...
            tryagain:
                ...
                infname = ii;
                while (infname != fullname && *infname != '\\')
                    infname--;
                if (*infname == '\\')
                {
                    *infname++ = 0;
...
void Driverpack::indexinf_ansi(wchar_t const *drpdir, wchar_t const *inffilename, const char *inf_base, size_t inf_len)
{
    ...
    if (range.first == range.second)
        Log.print_index("ERROR: missing [strings] in %S\n", inffull.Get());
    ...
    if (range.first == range.second)
        Log.print_index("ERROR: missing [version] in %S\n", inffull.Get());
    ...
    if (range.first == range.second)
        Log.print_index("ERROR: missing [manufacturer] in %S\n", inffull.Get());
            ...
            if (parse_info.parseField())
            {
                ...
                while (1)
                {
                    if (cur_manuf->sections_n > 1)
                        wsprintfA(secttry, "%s.%s", text_ind.get(strs[0]), text_ind.get(strs[cur_manuf->sections_n - 1]));
                    else
                    ...
                    if (range2.first == range2.second)
                        Log.print_index("ERROR: missing [%s] in %S\n", secttry, inffull.Get());
                    for (auto got2 = range2.first; got2 != range2.second; ++got2)
                    {
                        ...
                        while (parse_info2.parseItem())
                        {
                            ...
                            memcpy(installsection, s1b, s1e - s1b);
                            installsection[s1e - s1b] = 0;
                            ...
                            if (range3.first == range3.second)
                            {
                                if (cur_manuf->sections_n > 1)
                                {
                                    memcpy(installsection, s1b, s1e - s1b);
                                    installsection[s1e - s1b] = 0;
                                    strcat(installsection, ".");
                                    strcat(installsection, text_ind.get(strs[cur_manuf->sections_n - 1]));
                                }
                                else
                                {
                                    memcpy(installsection, s1b, s1e - s1b);
                            ...
                            *iii = 0;
                            ...
                            {
                                if (*iii)
                                    wsprintfA(installsection, "$%s", iii);
...
Driverpack::Driverpack(wchar_t const *driverpack_path, wchar_t const *driverpack_filename, Collection *col_v) : type(DRIVERPACK_TYPE_PENDING_SAVE), col(col_v)
{
...
int Driverpack::checkindex()
{
    ...
    if (memcmp(buf, "SDW", 3) != 0 || version != VER_INDEX)
        if (version != 0x204)
            return 0;
...
void Driverpack::saveindex()
{
    ...
    sz = inffile.size() * sizeof(data_inffile_t) + manufacturer_list.size() * sizeof(data_manufacturer_t) + desc_list.size() * sizeof(data_desc_t) +
         HWID_list.size() * sizeof(data_HWID_t) + text_ind.getSize() + indexes.getSize() * sizeof(Hashitem) + sizeof(int) + 6 * sizeof(int) * 2;
...
void Driverpack::genhashes()
{
    ...
    for (auto &it : inffile)
    {
        ...
        for (int j = CatalogFile; j <= CatalogFile_ntamd64; j++)
            if (it.fields[j])
            {
...
void Driverpack::print_index_hr()
{
    ...
    for (inffile_index = 0; inffile_index < n; inffile_index++)
    {
        ...
        for (i = 0; i < (int)n; i++)
            if (i != (int)inffile_index && d_i->infcrc == inffile[i].infcrc)
            ...
            if (manufacturer_list[manuf_index].inffile_index == inffile_index)
            {
                ...
                for (pos = 0; pos < manufacturer_list[manuf_index].sections_n; pos++)
                {
                    ...
                    for (desc_index = 0; desc_index < desc_list.size(); desc_index++)
                        if (desc_list[desc_index].manufacturer_index == manuf_index && desc_list[desc_index].sect_pos == pos)
                        {
                            for (HWID_index = HWID_index_last; HWID_index < HWID_list.size(); HWID_index++)
                                if (HWID_list[HWID_index].desc_index == desc_index)
                                {
                                    if (HWID_index_last + 1 != HWID_index && HWID_index)
                                    ...
                                    fprintf(f, "       %s %-50s%-20s\t%s\n", buf, hwidmatch.getdrp_drvHWID(), hwidmatch.getdrp_drvinstall(), hwidmatch.getdrp_drvdesc());
                                    fprintf(f, "          feature:%-42hX%-20s\n\n", hwidmatch.getdrp_drvfeature() & 0xFF, hwidmatch.getdrp_drvinstallPicked());
                                }
                                else if (HWID_index != HWID_index_last)
                                    break;
                        }
                }
            }
            else if (manuf_index != manuf_index_last)
                break;
    ...
    *inf_pos = -1;
    ...
    for (unsigned HWID_index = start_index; HWID_index < HWID_list.size(); HWID_index++)
    {
        if (!_strcmpi(text_ind.get(HWID_list[HWID_index].getHWID()), hwid))
        {
            ...
            if (!_strcmpi(hwidmatch.getdrp_drvinstallPicked(), sect) || StrStrIA(hwidmatch.getdrp_drvinstall(), sect))
            {
