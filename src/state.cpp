...
int State::getbaseboard(WStringShort &manuf1, WStringShort &model1, WStringShort &product1, WStringShort &cs_manuf1, WStringShort &cs_model1, int *type)
{
    *type = 0;
    ...
    hres = pSvc->ExecQuery(_bstr_t(L"WQL"), _bstr_t(L"SELECT * FROM Win32_BaseBoard"), WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, nullptr, &pEnumerator);
    if (FAILED(hres))
    {
    ...
    else
    {
        ...
        while (pEnumerator)
        {
            ...
            if (vtProp1.bstrVal)
                manuf1.strcpy(vtProp1.bstrVal);

            vtProp2.bstrVal = nullptr;
            hres            = pclsObj->Get(L"Model", 0, &vtProp2, nullptr, nullptr);
            if (vtProp2.bstrVal)
                model1.strcpy(vtProp2.bstrVal);

            vtProp3.bstrVal = nullptr;
            pclsObj->Get(L"Product", 0, &vtProp3, nullptr, nullptr);
            if (vtProp3.bstrVal)
                product1.strcpy(vtProp3.bstrVal);
        }
    }

    hres = pSvc->ExecQuery(_bstr_t(L"WQL"), _bstr_t(L"SELECT * FROM Win32_ComputerSystem"), WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, nullptr, &pEnumerator);
    if (FAILED(hres))
    {
    ...
    else
    {
        ...
        while (pEnumerator)
        {
            ...
            pclsObj->Get(L"Manufacturer", 0, &vtProp1, nullptr, nullptr);
            if (vtProp1.bstrVal)
                cs_manuf1.strcpy(vtProp1.bstrVal);

            vtProp2.bstrVal = nullptr;
            pclsObj->Get(L"Model", 0, &vtProp2, nullptr, nullptr);
            if (vtProp2.bstrVal)
                cs_model1.strcpy(vtProp2.bstrVal);
        }
    }

    hres = pSvc->ExecQuery(_bstr_t(L"WQL"), _bstr_t(L"SELECT * FROM Win32_SystemEnclosure"), WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, nullptr, &pEnumerator);
    if (FAILED(hres))
    {
